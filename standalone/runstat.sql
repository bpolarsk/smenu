...
--
--             Requires CREATE PROCEDURE to create the package pkg.
--
--             Requires CREATE PUBLIC SYNONYM to create the pkg public synonym.
--
--   MODIFICATIONS:
...
PROMPT >> (using Control-C or break) and refstart, otherwise press [Return]
ACCEPT rinstans
--
...
/
--
CREATE OR REPLACE PACKAGE BODY rs
AS
  g_fstart NUMBER;
  g_run1  NUMBER;
  g_run2  NUMBER;
  ...
  IS
  BEGIN
    g_run1 := (DBMS_UTILITY.get_time - g_fstart);
    --
    INSERT INTO run_stats 
  ...
  IS
  BEGIN
    g_run2 := (DBMS_UTILITY.get_time - g_fstart);
    --
    ...
    ( SELECT
        ...
        TO_CHAR( round( run1/decode(run2,0,0.00000001,run2)*100,2 ), '99,999.99' ) || '%' data
      FROM 
        (
          SELECT
  ...
  BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(CHR(9)||'    exec rs.fstart;');
    DBMS_OUTPUT.PUT_LINE(CHR(9)||'    exec rs.middle;');
    DBMS_OUTPUT.PUT_LINE(CHR(9)||'    exec rs.stop;');
    --
    DBMS_OUTPUT.put_line(CHR(9));
...
--
SET escape "^";
CREATE PUBLIC SYNONYM rs FOR &runstats_usr^.rs;
