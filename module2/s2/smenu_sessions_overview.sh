...
column_name_long=48
A_USERNAME=a18
USR_NOT_NULL=" and s.username is not NULL"
HOSTNAME=`uname -n`
ACTION=DEFAULT
ROWNUM=30000
function help {
cat <<EOF
    ...
    Show sessions in system

       sl             # Info on all sessions.                          |  sl <SID>       # Show info for one session
       sl -a          # List session with active status
       sl -r          # session actually running an sql                |      sl -o        # list under a short form
       sl -c          # show cpu and db time                           |      sl -ash [nn] # show waits from active session history.
       sl -t  <SID>   # show events impact breakdown on session        |                   # default is last 60 minutes
       sl -io <SID>   # Show session IO                                |      sl -cpu      # Report Response time breakdown
       sl -siz        # show session memory size                       |      sl -lw       # active session locking and waits (10g)
       sl -u  <user>  # limit selection to session owned by <username> |      sl -f        # Failed over sessions
       sl -i  <nn>    # program column length is set to <nn>           |      sl -m        # display machine name instead of last activity  
       sl -la <sid>   # list active session entries for session        |      sl -ltop  -mn<minutes>   # List top active session, default is 15 min

Additionals:
------------
          -rac       # target GV\$ instead of V\$                   -len       # len of the column program

 Example : sl -l r -rn 20       # list all session order by disk reads, limits to 20 first rows

Supend session:
---------------
       sl -sup <SID> <r|s>  # Suspend a session (freeze) s=suspend r=resume
	                    # Example : sl sup 45 s  -->suspend session 45  #    sl sup 45 r  -->resume session 45
  Session stats:

       sl -l [<x>]    # List summary stats. Optionaly order by [<x>]    p : hard parse     c : sort by CPU       r : physical read
                                                                        b : block change   g : gets             cc : consistent change
Dumps:
------
        sl -dump <SID> [ -pga |-pga_adr <adr1> <ard2> | -st ] # Dump session PGA stack, protion of PGA (adr) or Session process state
        sl -rf <trace_file>  -pga     # read dump of pga 


Snapper implementation:          sl -p <SID> -s <secs> -n <nn> -sw
-----------------------     

         Example :  sl -p 155 -s 1 
...
FIELD7="s.server, s.last_call_et ssa, substr(s.machine,1,instr(s.machine,'.')-1 )machine"
FIELD4="s.serial#,"
typeset -u FTYPE
while [ -n "$1" ]
do
  case $1 in
       ...
       -c ) ACTION="DB_CPU" ;;
       -f ) ACTION="SLF" ;;
      -io ) ACTION="SLIO"; SID=$2 ; shift ;;
     -cpu ) ACTION=CPU ;;
    -dump ) ACTION=DUMP ;  SID=$2 ; shift ;;
      -st ) DUMP_VAR=ST ;;
     -pga ) DUMP_VAR=PGA ;;
 -pga_adr ) DUMP_VAR=PGA_ADR ; START_ADR=$2 ; END_ADR=$3; shift; shift ;;
       -g ) GATHER="$GATHER$2" ;  shift ;;
       -i ) column_name_long=41 ;;
       -l )  ACTION=SLH  ; 
             if [ -n "$2" ];then
                case "$2" in
                 p ) ORDER=" order by t2.value desc" ; shift  ;;
                 c ) ORDER=" order by t.value desc"  ; shift ;;
                 r ) ORDER=" order by physical_reads desc"  ; shift ;;
                 g ) ORDER=" order by consistent_gets desc"  ; shift ;;
                 cc) ORDER=" order by consistent_changes desc"  ; shift ;;
                 b ) ORDER=" order by block_changes desc"  ; shift ;;
                esac
              fi
             ;;
     -la  ) ACTION=LIST_ACTIVE; SID=$2; shift ;;
     -ltop  ) ACTION=TOP_ACTIVE;;
     -len ) column_name_long=$1 ;;
      -lw ) ACTION="LOCK_WAIT" ;;
       -n ) SNAP_PAR3=$2 ; shift ;;
       -o ) ACTION=OVERVIEW;;
       -m ) FIELD7="substr(s.machine,1,instr(s.machine,'.')-1 )machine " ; FIELD4=service_name,;;
       -mn ) MINUTES=$2 ; shift ;;
       -p ) ACTION=SNAPER ; SID=$2; shift   ;;
    -plike) PLIKE=$2 ;shift ;;
       -r ) ACTION="RUNNING" ;;
      -rf ) ACTION=READ_DUMP_PGA ; TRC=$2 ; shift ;;
      -rn ) ROWNUM=$2 ; shift ;;
     -rac ) G=G ; unset FIELD4; EXIT=exit; INST_ID='s.inst_id,';;
       -s ) SNAP_PAR2=$2 ; shift ;;
       -sw ) SNAP_ASH="ash,"  ;;
     -siz ) ACTION=PSIZ 
            if [ -n "$2" ];then
               AND_SESSIONS=" and sid in ($2)"
               shift ;
            fi ;;
     -sup ) ACTION=SUSPEND; SID=$2 ; STYPE=$3; shift ;  shift ;;
       -t ) ACTION="EV_CPU"  ; SID=$2 ; shift;;
       -u ) unset USR_NOT_NULL ; A_USERNAME=a25 ; column_name_long=32
            AND_USER=" and upper(s.username) = upper('$2') " ; shift ;;
    -ulike) ULIKE=$2 ;shift ;;
       -v ) set -xv ;;
       -h ) help;;
        * ) SID=$1  ; AND_SID=" and s.sid = $1" ; ACTION=SLH ;;
  esac
  shift
done
...
# ------------------------------------------------------------------------------
#     Data from  active sessions history
# ------------------------------------------------------------------------------
if [ "$ACTION" = "TOP_ACTIVE" ];then
...
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||rpad(USER  ,15) || 'top active sessions : '
from sys.dual
/
set lines 210 
set head on
...
(
SELECT 
  s.username, s.module, s.sid, s.serial#, s.sql_id,count(*) count,
    round(sum(TM_DELTA_CPU_TIME)/1000,0) cpu_ms,
    sum(DELTA_READ_IO_REQUESTS) Reads,
    sum(DELTA_WRITE_IO_REQUESTS) writes,
     round(sum(TM_DELTA_DB_TIME)/1000,0) db_time_ms,
    round(sum(TIME_WAITED)/1000,0) waited_ms
FROM v\$active_session_history h, v\$session s
WHERE h.session_id = s.sid
AND h.session_serial# = s.serial#
AND session_state= 'ON CPU' AND
sample_time > sysdate - interval '$MINUTES' minute
...
#     Data from  active sessions history
# ------------------------------------------------------------------------------
elif [ "$ACTION" = "LIST_ACTIVE" ];then
echo $NN "MACHINE $HOST - ORACLE_SID : $ORACLE_SID $NC" 
sqlplus -s "$CONNECT_STRING" <<EOF
column nline newline
set pagesize 66  linesize 120  heading off  embedded off pause off  termout on  verify off
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||rpad(USER  ,15) || 'active sessions history : '
from sys.dual
/
set lines 230
set head on
...
col objn for a38 head 'Object_name'
col otype for a18 head 'Oject|type'
col fn for 9999
col fsid for a12 head 'Session'
col bsid for 9999
col lm for 99
col p3 for 99999999999
col p2 for 99999999999
col blockn for 99999999
col st head 'Time'
col xcur for a4 head 'exec|curr'
col bsid head 'Blocking|sess' justify c
col fbr for a20 head 'File.block.row'
col doing for a16
col machine for a14
col port for 99999
col reads for 99999
col writes for 9999
col cpu_ms for 9999
col db_time_ms for 9999
col SQL_PLAN_HASH_VALUE head plan for 99999999999
select * from (
select $INST_ID
    to_char(sample_time,'HH24:MI:SS') st,
    to_char(a.session_id) || ':'||to_char(SESSION_SERIAL#) fsid,
    to_char(SQL_EXEC_START,'HH24:MI:SS') exec_start,
    a.SQL_ID,  a.sql_exec_id, SQL_PLAN_HASH_VALUE,
    a.IS_SQLID_CURRENT xcur, 
    round(TM_DELTA_CPU_TIME/1000,0) cpu_ms,
    DELTA_READ_IO_REQUESTS Reads,
     DELTA_WRITE_IO_REQUESTS writes,
     round(TM_DELTA_DB_TIME/1000,0) db_time_ms,
    round(TIME_WAITED/1000,0) waited_ms,
    substr(event,0,20) event,
    nvl(decode(o.subobject_name,null,o.object_name,o.object_name||'.'||o.subobject_name),a.current_obj#) objn,
    to_Char(CURRENT_FILE#) ||'.'|| to_char(CURRENT_BLOCK#)|| '.' || to_char(CURRENT_ROW#) fbr,
    mod(a.p1,16)  lm,
    --substr(o.object_type,0,10) otype,
    case 
      when IN_PARSE ='Y' then 'IN_PARSE'
      when IN_HARD_PARSE = 'Y' then 'IN_HARD_PARSE'
      when IN_SQL_EXECUTION = 'Y' then 'IN_SQL_EXECUTION'
      when IN_PLSQL_EXECUTION = 'Y' then 'IN_PLSQL_EXECUTION'
      when IN_PLSQL_RPC = 'Y' then 'IN_PLSQL_RPC'
      when IN_PLSQL_COMPILATION = 'Y' then 'IN_PLSQL_COMPILATION'
      when IN_JAVA_EXECUTION = 'Y' then 'IN_JAVA_EXECUTION'
      when IN_BIND = 'Y' then 'IN_BIND'
      when IN_CURSOR_CLOSE = 'Y' then 'IN_CURSOR_CLOSE'
      when IN_SEQUENCE_LOAD = 'Y' then 'IN_SEQUENCE_LOAD'
-- 19c      when IN_INMEMORY_QUERY = 'Y' then 'IN_INMEMORY_QUERY'
-- 19c     when IN_INMEMORY_POPULATE = 'Y' then 'IN_INMEMORY_POPULATE'
-- 19c     when IN_TABLESPACE_ENCRYPTION = 'Y' then 'IN_TABLESPACE_ENCRYPTION'
      else 'Unknown'
   end Doing
   , BLOCKING_SESSION bsid  --, substr(machine,1,instr(machine,'.')-1 ) MACHINE, PORT
from
   ${G}v\$active_session_history a, all_objects o
where 
...
set pagesize 66  linesize 120  heading off  embedded off pause off  termout on  verify off
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||rpad(USER  ,15) || 'List active sessions  : Use ''obj -n <nn>'' to see name of ''obj id'''
from sys.dual
/
set term on verify off pagesize 333 feedback off  head on
set lines 220 pagesize 66 
col spid format 99999999 head 'Spid'
col sid format 99999 head 'Sid'
col username format a16
col EVENT for a30 head 'Event'
col lce for 9999999 head 'Secs in|Active'
col SECONDS_IN_WAIT form 999999 head 'Secs in|Wait' justify c
col bs form 9999 head 'Blk|ses'
col rwo for a30 head 'Obj|Id'
col inst_id for 9999 head 'Inst'
col machine for a12 truncate
col program for a35 truncate
col run_secs for 999999 head 'Run|secs' justify c
col wait_time for 9990 head 'Wait|time'
-- with v as ( select substr(version,1,instr(version,'.',1)-1) ora_version from v\$instance )
select $INST_ID sid, username, sql_id, LAST_CALL_ET lce,
         -- case when  ora_version < 11 then 0 else (SYSDATE-SQL_EXEC_START)*86400 end run_secs, 
               (SYSDATE-SQL_EXEC_START)*86400 run_secs,
       WAIT_TIME, SECONDS_IN_WAIT, STATE, EVENT,
       BLOCKING_SESSION bs, 
       decode (to_char(ROW_WAIT_OBJ#), '-1', '-1' , object_name ) rwo,
       program , machine
 from  ${G}v\$session s , dba_objects 
where 
       s.status = 'ACTIVE' $USR_NOT_NULL $AND_USER 
...
#select object_name from dba_objects where object_id = $OBJN;"

# ------------------------------------------------------------------------------
#     Report Response time breakdown
# ------------------------------------------------------------------------------
    ...
    if [ -n "$STYPE" ];then
          if [ "$STYPE" = 's' ];then
                ...
                echo "$TYPE should be 's' or 'r' : Invalid suspend/resume action"
                exit
          fi
     else
        echo "No suspend/resume action"
        exit
     fi
...
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||rpad(USER  ,15) || 'Suspend session - Session SID to suspend : $TO_SUSP '
from sys.dual
/
...
from sys.${G}v_\$session s, sys.${G}v_\$process p
where s.sid = $SID and
s.paddr = p.addr
and s.username != 'NULL'
order by 2
...
select p.spid from v\\$session s, v\\$process p where s.sid = $SID  and s.paddr = p.addr ;
EOF`
if $SBINS/yesno.sh "to $FACTION Session $SID" DO N
  then
sqlplus -s "$CONNECT_STRING" <<EOF
...
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  ,
       'Response time breakdown' nline
from sys.dual
/
set linesize 124 
set heading on
column major      format a8
column minor      format a25
column wait_event format a50 trunc
...
order by n_major, n_minor, time desc
/
EOF
exit
...
set embedded on heading on feedback off pause off linesize 110 pagesize 66

col sname format A16 head "User name"
col type format A10 head "object|type"
col obj_name format A30 head "object|name"
col sid format 99999 head "sid"
col gets format 9,999,999,990 head "Gets"
col reads format 9,999,999,990 head "Physical|reads"
col chg  format 9,999,999,990 head "Changes"

break on sname on sid on gets on reads on chg

select s.username sname,v.sid ,
i.BLOCK_CHANGES + I.CONSISTENT_CHANGES gets,
 I.PHYSICAL_READS reads,I.BLOCK_CHANGES + I.CONSISTENT_CHANGES chg,
v.type,v.object obj_name
from sys.${G}v_\$session s, sys.${G}v_\$access v, V\$SESS_IO I
where v.sid = s.sid 
  and s.sid = $SID 
  and v.sid = i.sid
order by gets desc
...
# ------------------------------------------------------------------------------
#    Failed over sessions
# ------------------------------------------------------------------------------
elif [ "$ACTION" = "SLF" ];then
echo "MACHINE $HOSTNAME - ORACLE_SID : $ORACLE_SID                   Page: 1"
        sqlplus -s "$CONNECT_STRING" <<EOF
column nline newline
set linesize 157 pagesize 66 heading off pause off embedded off verify off
spool $FOUT
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  nline ,
       'Sessions          -  List failed over sessions info ' nline
        from sys.dual
/
set embedded on
rem prompt *----------------------------------------------------*
...
column "terminal" format a6
column "OSUSER" format a12
column "OS-PPID" format a8
column "sess" format 9999 heading "Sess|ID" justify c
column "serial#" format 99999 heading "Serial" justify c
column "PID" format 99999 heading "PID#" justify c
column "ssa" format 999,999 heading "Last|activity" justify c
column "failed_over" format A10 heading "Failed| Over"
column "failover_method" format A14 heading "Fail. method"
column "failover_type" format A14 heading "Fail. type"

select nvl(s.username,p.program) "USERNAME",s.sid sess ,s.OSUSER,
       s.serial#,p.spid "PID", s.process "OS-PPID", s.last_call_et ssa, s.status,
       failed_over, failover_type,failover_method, server
from sys.${G}v_\$session s, sys.${G}v_\$process p
...
#    Read pga dump
# ------------------------------------------------------------------------------
elif [ "$ACTION" = "READ_DUMP_PGA" ];then
     if [ ! -f "$TRC" ];then
     ...
     fname30=${fname30:0:30}               # trunc file name to max 30 characters
...
--  my local database 10gR2
--  get_trace_file script should be executed beforehand
-- @usage: heap_analyze <file_name>
-- Adapted to Smenu by bpa 10:06:2009 
--------------------------------------------------------
set verify off lines 190 pages 66
column tsize new_value total_size
column ratio format 99.99
column ds format a20
col ctype for a30
col atype for a30
with t as (
    select 
        to_number(substr(fline, 21)) as tsize
    from 
         $fname30
    where
       regexp_like(fline, 'Total heap size')
) 
select
    sum(tsize) as tsize
from t ;
...
with c as (
    select atype, csize, sum(csize) over() as t_size, &total_size as heap_size
    from (
        select atype, sum(csize) as csize
        from (
            select
               to_number(regexp_replace(fline,'.*sz=[[:space:]]+([[:digit:]]+)[[:space:]].*','\1')) csize,
               rtrim(regexp_replace(fline,'.*sz=[[:space:]]+[[:digit:]]+[[:space:]]+([[:alnum:]]+)[[:space:]].*','\1')) atype,
               rtrim(regexp_replace(fline,'.*["]+(.*)["].*','\1')) ctype,
               rtrim(regexp_replace(fline,'.*["]+.*["](.*)','\1')) ds
            from 
                 $fname30
            where 
                substr(fline, 1, 7) = '  Chunk'
            and rtrim(regexp_replace(fline,'.*sz=[[:space:]]+[[:digit:]]+[[:space:]]+([[:alnum:]]+)[[:space:]].*','\1')) <> 'free'
            ) 
        group by atype
    )
)
select atype, csize, heap_size, 
    (csize/heap_size)*100  as ratio
from c
union all
select 'free', heap_size-t_size, heap_size, ((heap_size-t_size)/heap_size)*100 as ratio
from c where rownum = 1 ;

with c as (
    select ctype, csize, sum(csize) over() as t_size, &total_size as heap_size
    from (
        select ctype, sum(csize) as csize
        from (
            select
               to_number(regexp_replace(fline,'.*sz=[[:space:]]+([[:digit:]]+)[[:space:]].*','\1')) csize,
               rtrim(regexp_replace(fline,'.*sz=[[:space:]]+[[:digit:]]+[[:space:]]+([[:alnum:]]+)[[:space:]].*','\1')) atype,
...
select ctype, csize, heap_size, (csize/heap_size)*100  as ratio
from c
union all
select 'free', heap_size-t_size, heap_size, 
    ((heap_size-t_size)/heap_size)*100 as ratio
from c where rownum = 1 ;

with c as (
    select ds, csize, sum(csize) over() as t_size, &total_size as heap_size
    from (
        select ds, sum(csize) as csize
        from (
            select
               to_number(regexp_replace(fline,'.*sz=[[:space:]]+([[:digit:]]+)[[:space:]].*','\1')) csize,
               rtrim(regexp_replace(fline,'.*sz=[[:space:]]+[[:digit:]]+[[:space:]]+([[:alnum:]]+)[[:space:]].*','\1')) atype,
               rtrim(regexp_replace(fline,'.*["]+(.*)["].*','\1')) ctype,
               rtrim(regexp_replace(fline,'.*["]+.*["][[:space:]]+ds=(.*)','\1')) ds 
            from 
                $fname30
            where 
                substr(fline, 1, 7) = '  Chunk'
                and regexp_like(fline, 'ds=' )
            ) 
        group by ds
    )
)
select
     ds, csize, heap_size, (csize/heap_size)*100  as ratio from c
union all
select 'free', heap_size-t_size, heap_size, ((heap_size-t_size)/heap_size)*100 as ratio
from c 
    where rownum = 1 ;
EOF
 ...
 # drop now the external table.
var=`sqlplus -s "$CONNECT_STRING" <<EOF
set feed off pagesize 0 head off
select count(1) from dba_external_locations where table_name = upper('$fname30') and LOCATION='$fname' ;
EOF`
if  [ $var -gt 0 ];then
    sqlplus -s "$CONNECT_STRING" <<EOF
    set feed off pagesize 0 head off pause off
    drop table $fname30;
EOF
fi

# ------------------------------------------------------------------------------
#       Dump PGA and process state
# ------------------------------------------------------------------------------
elif [ "$ACTION" = "DUMP" ];then
   if [ "$DUMP_VAR" = 'ST' ];then
         DUMP=" processstate 10"
   elif [ "$DUMP_VAR" = 'PGA_ADR' ];then
         DUMP="heapdump_addr $START_ADR $END_ADR"
   else
         DUMP="heapdump 1"
   fi
(
...
set pagesize 66 linesize 80 heading off embedded off pause off termout on verify off
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  nline , 'Session SID whose dump : $TO_DUMP ' nline from sys.dual
/
rem set pages 0 feed off echo off
set embedded on
rem prompt *----------------------------------------------------*
...
select nvl(s.username,p.program) "USERNAME",s.sid sess ,s.OSUSER,
       s.serial#,p.spid "PID", s.process "OS-PPID", s.program
from sys.${G}v_\$session s, sys.${G}v_\$process p
where s.sid = $SID and s.paddr = p.addr and s.username != 'NULL' order by 2
/
EOF
)
...
if $SBINS/yesno.sh "to dump PGA for Session $TO_DUMP" DO N
  then
     trc_file=`sqlplus -s "$CONNECT_STRING" <<EOF
...
EOF`
     echo "$trc_file"
     trc=`echo "$trc_file"|tail -1`
     if [ "$DUMP_VAR" = "PGA" ];then
        echo
        echo "Use : 'sl -rf $trc'   to start parsing this trace file"
        echo
     fi
fi
# ------------------------------------------------------------------------------
elif [ "$ACTION" = "SLH" ];then
sqlplus -s "$CONNECT_STRING" <<EOF
...
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  nline ,
       'Session Hit (HELP slh -h)' nline from sys.dual
/
set heading on linesize 190 pagesize 66
col sid format 99999 heading "Sid"
col hit format A4 heading "%Hit"
col bchng format 9999999999 head "Blocks |Changes" justify c
col cchng format 999999999 head "Consistent|Changes" justify c
col cgets format 9999999999 head "Consistent|Gets" justify c
col cpu   format 9990.99 head "Cpu" justify c
col Pread head "Physical|Reads" justify c
col hp format 999999 head "Hard|Parse" justify c
col username format A22 head "username" justify c
col osuser format A12 head "osuser" justify c
col ospid format A10 head "ospid" justify c
col com format A3 head "Com"
col typ format A4 head "User|Type"
col waits head 'waits(s)' format 999990.0

select sid, username, ospid , com ,sql_id, typ, CPU, HP, bchng,cchng,cgets,pread, hit, waits
from (
select s.sid sid,
       decode(s.username,null, 
                         '-'||regexp_replace(s.program , '.*(\(.*\)).*','\1') ,
              s.Username) username, osuser ,
       osuser
       ,spid ospid,
       ...
       t2.value HP,
       block_changes bchng,
       consistent_changes cchng,
       consistent_gets cgets,
       physical_reads pread,
       substr(decode((consistent_gets+block_gets),0,'None',
             (100*(consistent_gets+block_gets-physical_reads)/
             (consistent_gets+block_gets))),1,4) hit,
       (select sum(TIME_WAITED) waited from v\$session_event where wait_class# in (0,1,2,4,8) and sid=s.sid)/100 waits
from sys.${G}v_\$process p, sys.${G}v_\$SESSTAT t,sys.${G}v_\$sess_io i ,
     sys.${G}v_\$session s, sys.${G}v_\$statname n, sys.${G}v_\$sesstat t2
where  
       i.sid=s.sid and p.addr=paddr(+) 
  and  s.sid=t.sid $AND_USER 
       and t.statistic# (+) = 12   
  and t2.sid (+) = s.sid 
  and t2.statistic# = n.statistic# 
  and n.name (+) = 'parse count (hard)'  $AND_SID $ORDER
) where rownum < $ROWNUM
/
EOF
exit
# ------------------------------------------------------------------------------
# Snapper -- Session Snapper v1.06 by Tanel Poder ( http://www.tanelpoder.com )
# ------------------------------------------------------------------------------
elif [ "$ACTION" = "SNAPER" ];then
   #SNAP_PAR1="ash=event+wait_class,stats,gather=ts,tinclude=CPU,sinclude=parse"
   SNAP_PAR1=$SNAP_ASH"stats,gather=tsw"
   SNAP_PAR2=${SNAP_PAR2:-1}
   SNAP_PAR3=${SNAP_PAR3:-1}
   SNAP_PAR4=$SID

sqlplus -s "$CONNECT_STRING" <<EOF
set termout off tab off verify off linesize 200

-- Get parameters
define snapper_options='$SNAP_PAR1'
define   snapper_sleep='$SNAP_PAR2'
define   snapper_count='$SNAP_PAR3'
define     snapper_sid='$SNAP_PAR4'
...
set termout off head off feed off
declare
...
from
    mod_banner
/
set termout on serveroutput on size 1000000 format wrapped head on feed on
...
&_IF_ORA9207_OR_HIGHER      output_pcthist    number := 0; -- percent of total visual bar (histogram) -- Histograms seem to work for me on 9.2.0.7 + - JBJ2)
...
EOF
# ----------------
# Events and CPU time for a session
# ----------------
elif [ "$ACTION" = "EV_CPU" ];then
 if [ -z "$SID" ];then
      echo
      echo
      echo "  Error : I need an Session SID\n"  
      echo "          sl -t 1 "
      echo
      echo
      exit
...
echo "MACHINE $HOSTNAME - ORACLE_SID : $ORACLE_SID                   Page: 1"
sqlplus -s "$CONNECT_STRING" <<EOF
set lines 132 pagesize 66 feed off pagesize 30 verify off
...
col impact format 990.9
col service_time head 'Service Time (cs)'
col tot_wait head 'Total waits|For session (cs)'
col response_time head 'Total | Response time(cs)' justify c
col cpu_other head 'cpu others (cs)'
col recursive_cpu head 'recursive |cpu(cs)'  justify c
col parse_time head 'Parse | time (cs)'  justify c
col impact head 'Impact (%)'

select  service_time, tot_wait , (service_time + tot_wait) response_time, 
       (service_time-parse_time-recursive_cpu) cpu_other , recursive_cpu, parse_time
from 
   ( select value service_time FROM sys.${G}v_\$sesstat st, sys.${G}v_\$statname sn 
            where st.sid=$SID and st.statistic# = sn.statistic# and sn.name = 'CPU used by this session' ),
   ( select value parse_time FROM sys.${G}v_\$sesstat st, sys.${G}v_\$statname sn 
            where st.sid=$SID and st.statistic# = sn.statistic# and sn.name = 'parse time cpu'  ),
   ( select value recursive_cpu FROM sys.${G}v_\$sesstat st, sys.${G}v_\$statname sn 
            where st.sid=$SID and st.statistic# = sn.statistic# and sn.name = 'recursive cpu usage' ),
   ( select  sum(se.time_waited) tot_wait from sys.${G}v_\$session_event se 
             where se.sid=$SID and se.wait_class != 'Idle' )
/

prompt
prompt =================================================================

prompt =  Break down of session events and impact on the response time =
prompt =================================================================
...
select
  ...
  a.event, total_waits $S total_waits, total_timeouts , 
   a.time_waited,  a.average_wait, a.wait_class , decode(&response_time,0,0,time_waited/&response_time*100) impact
from
  sys.v_\$session_event a , sys.v_\$session b
  where a.sid = b.sid 
        and a.wait_class !='Idle'   and a.sid = '$SID'
        order by impact desc;
prompt
col  tim head 'Time (ms)'
select /*+ leading(mys) no_merge(mys) */
        stat_name, round(value/1024/1024,1) tim
from
        sys.${G}v_\$sess_time_model stm
where
        stm.sid = $SID
and     stm.value != 0 ;
EOF
#--select value dbtime FROM sys.${G}v_\$sesstat st, sys.${G}v_\$statname sn where st.sid=$SID and st.statistic# = sn.statistic# and sn.name = 'DB time' ;
# ----------------
# CPU and DB time
# ----------------
elif [ "$ACTION" = "DB_CPU" ];then
sqlplus -s "$CONNECT_STRING" <<EOF
set lines 132 pagesize 66 
col username format a16
col pp format  a8 head 'OS PID'
col module format a50
col sid format 99999
SELECT s.sid, p.spid pp, s.username, s.module, st.value/100 as "DB Time (sec)"
    , stcpu.value/100 as "CPU Time (sec)", decode(st.value,0, round(stcpu.value / st.value * 100,2)) as "% CPU"
    FROM sys.${G}v_\$sesstat st, sys.${G}v_\$statname sn, sys.${G}v_\$session s, sys.${G}v_\$sesstat stcpu, sys.${G}v_\$statname sncpu, sys.${G}v_\$process p
    WHERE sn.name = 'DB time' -- CPU
    AND st.statistic# = sn.statistic# $AND_USER and
        st.sid = s.sid
...
# ASH
# ----------------
elif [ "$ACTION" = "ASH" ];then
sqlplus -s "$CONNECT_STRING" <<EOF
set lines 190 pagesize 66 
col username format a16
col event form a30
col sst head 'Session|State'
col BLOCKING_SESSION for 99999 head 'Block|sess'
col CURRENT_OBJ#  head obj#
col object_name for a18
break on report on session_id on sst

 SELECT  a.session_id,s.username, 
         -- (select trunc(mod(sum((instr('0123456789abcdfghjkmnpqrstuvwxyz', substr(lower(trim(a.sql_id)),level,1))-1)
         -- *power(32,length(trim(a.sql_id))-level)),power(2,32))) hash_value 
         --      from dual connect by level <= length(trim(a.sql_id)) ) hash_value,
         a.sql_id,
         DECODE (a.session_state, 'WAITING', a.event, NULL) event,
                     a.session_state sst , COUNT(*), SUM (a.time_waited) time_waited,
         a.BLOCKING_SESSION , o.object_name  , substr(t.sql_text,1,40) sql
        FROM     sys.${G}v_\$active_session_history a, sys.${G}v_\$session s , all_objects o, sys.v_\$sqltext t 
        WHERE    a.session_id =  s.sid   and 
                 a.SESSION_SERIAL# = s.serial# and
                 a.sample_time >= SYSDATE -  ($MIN / 1440)  and
                 a.current_obj# = o.object_id (+) and
                 a.sql_id = t.sql_id (+)
                 and t.piece (+) =0
            GROUP BY DECODE (a.session_state, 'WAITING', a.event, NULL), 
                  a.session_id, a.sql_id, s.username, a.session_state,  a.BLOCKING_SESSION , o.object_name, substr(t.sql_text,1,40)
            ORDER by 1;
EOF
# ----------------
# Size
# ----------------
elif [ "$ACTION" = "PSIZ" ];then
echo "MACHINE $HOSTNAME - ORACLE_SID : $ORACLE_SID                   Page: 1"
        sqlplus -s "$CONNECT_STRING" <<EOF
column nline newline
set linesize 80 pagesize 66 heading off pause off embedded off verify off
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  nline , 'Sessions          -  Session memory size (help: sl -h)' nline
        from sys.dual
/
set embedded on linesize 124 pagesize 66 head on 
col  program format a50
break on report
comp sum label Total of alloc_mb used_mb max_mb on report
col alloc_mb head 'PGA curr.|Allocated' for 999990.9 justify c
col used_mb head 'PGA curr.|Used' for 999990.9 justify c
col max_mb head 'PGA Max.|Used' for 999990.9 justify c
col pga_freeable_mem head 'PGA Freeable' for 999990.9 justify c
select
   $INST_ID s.sid, p.program,  
   pga_alloc_mem/1048576 as alloc_mb,
   pga_used_mem/1048576 as used_mb,
   pga_max_mem/1048576 as max_mb,
   background, pga_freeable_mem/1048576 pga_freeable_mem
from sys.${G}v_\$process p ,sys.${G}v_\$session  s
  where s.paddr = p.addr $AND_SESSIONS 
...
set linesize 80 pagesize 66 heading off pause off embedded off verify off
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  nline , 'Sessions          -  Generic info Overview  (help: sl -h)' nline
        from sys.dual
/
set embedded on
rem prompt *----------------------------------------------------*
set lines 190
set heading on
column "USERNAME" format $A_USERNAME
column "terminal" format a6
column "OSUSER" format a16
column "sess" format 99999 heading "Sess|ID" justify c
column "serial#" format 99999 heading "Serial" justify c
column PID format A8 heading "PID#" justify c
column PPID format A16 heading "OS-PID" justify c
column "ssa" format 99,999,999 heading "Last|activity" justify c
...
select $INST_ID nvl(s.username,p.program) "USERNAME",s.sid sess ,s.OSUSER,
       $FIELD4 p.spid PID, s.process PPID,  $FIELD7
        , s.status,
       to_char(s.logon_time,'dd-mon hh24:mi') logon_time,s.program
from 
     sys.${G}v_\$session s, 
     sys.${G}v_\$process p
where s.paddr = p.addr (+) $AND_USER $USR_NOT_NULL $AND_SID order by $INST_ID s.sid 
/
$EXIT
-- prompt waiting sessions :
-- prompt
-- select sid,event from sys.${G}v_\$session_wait s where wait_time=0 $AND_SID and
--       (event not like '%message%' and event not like '%timer%' and event not like '%pipe%' and event not like '%wakeup%'  
--        and event not like '%idle wait%' and event not like '%class slave wait%' and event not like '%waiting for time management%')
-- /
prompt
prompt who is doing full_table_scans ?
prompt ===============================
prompt
select sid from sys.${G}v_\$session_wait where event like '%scattered%'
/
exit
/
...
# lock wait
# -----------
elif [ "$ACTION" = "LOCK_WAIT" ];then
        echo "MACHINE $HOSTNAME - ORACLE_SID : $ORACLE_SID                   Page: 1"
        sqlplus -s "$CONNECT_STRING" <<EOF
column nline newline
set linesize 80 pagesize 66 heading off pause off embedded off verify off
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  nline , 'Sessions          -  Session actually running an SQL  (help: sl -h)' nline
        from sys.dual
/
set term on verify off pagesize 333 linesize 190 feedback on  head on
col blocking_Session format 9999 head "block|sess"
col type head "Lock type"
col lmode head "Lock|held" format 999
col request head "lock|req" format 999
col object_name head "Name" format a20
col object_type head "Obj type" format a8
col event format a30
col sid format 99999
col seconds_in_Wait format 99999 head 'sec in| wait' justify c
select $INST_ID s.sid,sql_id,owner||'.'||nvl(subobject_name,object_name) object_name,
       decode(object_type, 'TABLE PARTITION', 'TBL PART','INDEX PARTITION','IDX PART',object_type)  object_type,
       lockwait,BLOCKING_SESSION, event , seconds_in_wait,v.type,Lmode, request
       from sys.${G}v_\$session s , dba_objects , sys.${G}v_\$lock v, sys.${G}v_\$process p
...
# --------------
    ...
    if [ -n "$G" ];then
         AND_INST=" and inst_id = s.inst_id "
    fi
        echo "MACHINE $HOSTNAME - ORACLE_SID : $ORACLE_SID                   Page: 1"
        sqlplus -s "$CONNECT_STRING" <<EOF
column nline newline
...
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  nline , 'Sessions          -  Session actually running an SQL  (help: sl -h)' nline
        from sys.dual
/
set term on verify off pagesize 333 linesize 124 feedback off  head on
col sql_text format a80
col sql_child_number head 'Child' for 999999
set timing on
set lines 190 pages 90
with v0 as ( select substr(version,1,instr(version,'.',1)-1) ora_version from v\$instance )
   select $INST_ID sid,v.sql_id, 
          case 
               when  ora_version < 11 then 0
               else (SYSDATE-SQL_EXEC_START)*86400
          end run_secs, 
          command,sql_text, s.sql_child_number
   from
               sys.${G}v_\$session s ,
               sys.${G}v_\$process p ,
               sys.${G}v_\$sql v, v0
  where p.addr=s.paddr      and
            background is null  and
            wait_time != 0 and -- seconds_in_Wait=0         and
            v.sql_id= s.sql_id
            and s.sql_child_number in (select min (child_number) from sys.${G}v_\$sql where s.sql_id = s.sql_id $AND_INST) ;
EOF
# ---------
# Overview
# ---------
...
select 'Date              -  '||to_char(sysdate,'Day Ddth Month YYYY  HH24:MI:SS'),
       'Username          -  '||USER  nline , 'Sessions          -  Generic info Overview  (help: sl -h)' nline
        from sys.dual
/
set term on verify off pagesize 333 linesize 210 feedback off  head on
...
column PROGRAM  format A55 trunc
column BACKGROUND       format A1
prompt B=Background Process
select $INST_ID decode(nvl(p.background, 0), 0, ' ', 'B') background, s.sid,to_number(p.SPID) SPID, p.USERNAME u_user,
        lower(s.USERNAME) o_user, p.TERMINAL, upper(decode(nvl(s.command, 0), 0,      '---------------',
        1, 'Create Table', 2, 'Insert ...', 3, 'Select ...', 4, 'Create Cluster', 5, 'Alter Cluster', 6, 'Update ...',
        7, 'Delete ...', 8, 'Drop ...', 9, 'Create Index', 10, 'Drop Index', 11, 'Alter Index', 12, 'Drop Table',
        13, '--', 14, '--', 15, 'Alter Table', 16, '--', 17, 'Grant', 18, 'Revoke', 19, 'Create Synonym',
        20, 'Drop Synonym', 21, 'Create View', 22, 'Drop View', 23, '--', 24, '--', 25, '--', 26, 'Lock Table',
        27, 'No Operation', 28, 'Rename', 29, 'Comment', 30, 'Audit', 31, 'NoAudit', 32, 'Create Ext DB',
        33, 'Drop Ext. DB', 34, 'Create Database', 35, 'Alter Database', 36, 'Create RBS', 37, 'Alter RBS', 38, 'Drop RBS',
        39, 'Create Tablespace', 40, 'Alter Tablespace', 41, 'Drop tablespace', 42, 'Alter Session', 43, 'Alter User',
        44, 'Commit', 45, 'Rollback', 46, 'Savepoint',47,'PL/SQL')) Cmdtype, s.program program, s.server, 
        substr(s.machine,1,instr(s.machine,'.')-1 )machine
from    sys.${G}v_\$process p, sys.${G}v_\$session s
where   p.addr=s.paddr (+) $AND_USER and p.spid is not null order by 1, program, p.username, s.username, spid;
EOF
else
  help
fi
